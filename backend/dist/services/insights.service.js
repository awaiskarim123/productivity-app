...
async function generateWeeklyInsights(prisma, userId, weekStart, weekEnd) {
    ...
    workSessions.forEach((session) => {
        const hour = (0, dayjs_1.default)(session.startedAt).hour();
    ...
    workSessions.forEach((session) => {
        const dayIdx = (0, dayjs_1.default)(session.startedAt).day();
    ...
    const averageDailyMinutes = Array.from(dayProductivity.values()).reduce((sum, val) => sum + val, 0) / 7;
    ...
    const habitCorrelations = habits.map((habit) => {
        const habitLogDays = new Set(habit.logs.map((log) => (0, dayjs_1.default)(log.date).format("YYYY-MM-DD")));
        ...
        workSessions.forEach((session) => {
            const sessionDate = (0, dayjs_1.default)(session.startedAt).format("YYYY-MM-DD");
    ...
    const completedFocusSessions = focusSessions.filter((s) => s.mode === enums_1.FocusSessionMode.FOCUS && s.completed).length;
    const focusCompletionRate = focusSessions.filter((s) => s.mode === enums_1.FocusSessionMode.FOCUS).length > 0
        ? completedFocusSessions /
            focusSessions.filter((s) => s.mode === enums_1.FocusSessionMode.FOCUS).length
        : 0;
    ...
    const averageFocusMinutes = focusSessions.filter((s) => s.mode === enums_1.FocusSessionMode.FOCUS).length > 0
        ? focusSessions
            .filter((s) => s.mode === enums_1.FocusSessionMode.FOCUS)
    ...
    const breakSessions = focusSessions.filter((s) => s.mode === enums_1.FocusSessionMode.BREAK);
...
async function getOrGenerateWeeklyInsights(prisma, userId, weekStart) {
    ...
    if (existing) {
        return {
            ...
            weekOverWeekTrend: existing.weekOverWeekTrend,
            ...
            habitCorrelations: existing.habitCorrelations,
