...
async function calculateGoalProgress(prisma, goalId) {
    ...
    if (goal.habits.length > 0) {
        const habitProgresses = goal.habits.map((habit) => {
            const logsInPeriod = habit.logs.filter((log) => {
                const logDate = (0, dayjs_1.default)(log.date);
    ...
    const focusProgress = estimatedTargetMinutes > 0 ? Math.min(100, (totalFocusMinutes / estimatedTargetMinutes) * 100) : 0;
 ...
 */
function calculateHealthStatus(progressPercent, timeProgress, remainingDays, totalDays) {
 ...
 */
async function updateGoalProgress(prisma, goalId) {
...
async function getGoalContributions(prisma, goalId) {
    ...
    const habitLogsInPeriod = goal.habits.flatMap((habit) => habit.logs.filter((log) => {
        const logDate = (0, dayjs_1.default)(log.date);
        return ((logDate.isAfter(startDate) || logDate.isSame(startDate, "day")) &&
    ...
    while (current.isBefore(endDate) || current.isSame(endDate, interval)) {
        ...
        const tasksCompleted = goal.tasks.filter((t) => {
            if (!t.completed || !t.completedAt)
                return false;
            const completedDate = (0, dayjs_1.default)(t.completedAt);
