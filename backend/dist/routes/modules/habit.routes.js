 ...
 */
async function calculateHabitStreak(prisma, habitId) {
    ...
    logs.forEach((log) => {
        const dateKey = dayjs_1.default.utc(log.date).startOf("day").toISOString();
...
async function habitRoutes(app) {
    app.addHook("preHandler", app.authenticate);
    app.post("/", async (request, reply) => {
        const result = habit_schema_1.createHabitSchema.safeParse(request.body ?? {});
        if (!result.success) {
    ...
    app.get("/", async (request, reply) => {
        const result = habit_schema_1.habitsQuerySchema.safeParse(request.query ?? {});
        if (!result.success) {
    ...
    app.post("/:id/log", async (request, reply) => {
        ...
        const logDate = result.data.date ? dayjs_1.default.utc(result.data.date) : dayjs_1.default.utc();
        ...
        const { streak, bestStreak } = await calculateHabitStreak(app.prisma, id);
    ...
    app.patch("/:id", async (request, reply) => {
        ...
        if (result.data.icon !== undefined)
            updateData.icon = result.data.icon;
        if (result.data.targetDays !== undefined)
            updateData.targetDays = result.data.targetDays;
        if (result.data.isActive !== undefined)
