<script lang="ts">
	...
	function getImpactColor(impact: 'positive' | 'neutral' | 'negative'): string {
		switch (impact) {
			...
			case 'neutral':
				return 'bg-gray-50 dark:bg-slate-500/20 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-500/40';
		}
	}
...
<div class="min-w-0 rounded-2xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-3 sm:p-4 shadow-sm dark:shadow-none">
	<div class="mb-3 flex items-center justify-between">
		<div>
			...
			<p class="mt-1 text-xs text-gray-600 dark:text-slate-400">How your habits affect productivity</p>
		</div>
		<button
			type="button"
	...
	{#if loading}
		<div class="py-4 text-center text-sm text-gray-600 dark:text-slate-400">Loading habit impact...</div>
	{:else if habitCorrelations.length > 0}
		<div class="space-y-3">
			<!-- Positive Impact Habits -->
			{#if positiveHabits.length > 0}
				<div>
					<h3 class="mb-2 text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-400">
						Positive Impact ({positiveHabits.length})
					</h3>
					<div class="space-y-2">
						{#each positiveHabits as habit}
							<div class="flex items-center justify-between rounded-xl border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/10 px-3 py-2">
								<div class="flex items-center gap-2">
									<span class="text-sm font-semibold text-emerald-700 dark:text-emerald-300">{getImpactIcon(habit.impact)}</span>
									<span class="text-sm text-gray-900 dark:text-slate-100">{habit.habitName}</span>
								</div>
								<span class="text-xs text-emerald-700 dark:text-emerald-300">+{Math.abs(habit.correlationScore * 100).toFixed(0)}%</span>
							</div>
						{/each}
					</div>
				</div>
			...
			{#if negativeHabits.length > 0}
				<div>
					<h3 class="mb-2 text-xs font-medium uppercase tracking-wide text-rose-700 dark:text-rose-400">
						Negative Impact ({negativeHabits.length})
					</h3>
					<div class="space-y-2">
						{#each negativeHabits as habit}
							<div class="flex items-center justify-between rounded-xl border border-rose-200 dark:border-rose-500/20 bg-rose-50 dark:bg-rose-500/10 px-3 py-2">
								<div class="flex items-center gap-2">
									<span class="text-sm font-semibold text-rose-700 dark:text-rose-300">{getImpactIcon(habit.impact)}</span>
									<span class="text-sm text-gray-900 dark:text-slate-100">{habit.habitName}</span>
								</div>
								<span class="text-xs text-rose-700 dark:text-rose-300">{habit.correlationScore * 100 < 0 ? '' : '-'}{Math.abs(habit.correlationScore * 100).toFixed(0)}%</span>
							</div>
						{/each}
					</div>
				</div>
			...
			{#if neutralHabits.length > 0}
				<div>
					<h3 class="mb-2 text-xs font-medium uppercase tracking-wide text-gray-600 dark:text-slate-400">
						Neutral Impact ({neutralHabits.length})
					</h3>
					<div class="space-y-2">
						{#each neutralHabits as habit}
							<div class="flex items-center justify-between rounded-xl border border-gray-200 dark:border-slate-800/60 bg-gray-50 dark:bg-slate-950/40 px-3 py-2">
								<div class="flex items-center gap-2">
									<span class="text-sm font-semibold text-gray-600 dark:text-slate-400">{getImpactIcon(habit.impact)}</span>
									<span class="text-sm text-gray-700 dark:text-slate-300">{habit.habitName}</span>
								</div>
								<span class="text-xs text-gray-600 dark:text-slate-400">~0%</span>
							</div>
						{/each}
					</div>
				</div>
