<script lang="ts">
	...
	function getContributionColor(contribution: number) {
		if (contribution >= 70) return 'text-emerald-600 dark:text-emerald-400';
		if (contribution >= 40) return 'text-yellow-600 dark:text-yellow-400';
	...
	function getPriorityColor(priority: string) {
		switch (priority) {
			case 'URGENT':
				return 'bg-rose-50 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-transparent';
			case 'HIGH':
				return 'bg-orange-50 dark:bg-orange-500/20 text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-transparent';
			case 'MEDIUM':
				return 'bg-yellow-50 dark:bg-yellow-500/20 text-yellow-700 dark:text-yellow-300 border border-yellow-200 dark:border-transparent';
			case 'LOW':
				return 'bg-blue-50 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-transparent';
			default:
				return 'bg-gray-50 dark:bg-slate-500/20 text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-transparent';
		}
	}
</script>

<div class="space-y-4">
	<h3 class="text-lg font-semibold text-gray-900 dark:text-white">What Contributed Most?</h3>

	{#if loading}
		<div class="text-center py-8">
			<div class="inline-block h-8 w-8 animate-spin rounded-full border-4 border-slate-700 border-t-emerald-500"></div>
			<p class="mt-2 text-sm text-gray-600 dark:text-slate-400">Loading contributions...</p>
		</div>
	{:else if error}
	...
	{:else if contributions}
		<div class="space-y-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 shadow-sm dark:shadow-none">
					<div class="text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Tasks</div>
				...
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 shadow-sm dark:shadow-none">
					<div class="text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Habits</div>
				...
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 shadow-sm dark:shadow-none">
					<div class="text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Focus Sessions</div>
				...
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 shadow-sm dark:shadow-none">
					<div class="text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Key Results</div>
			...
			{#if contributions.tasks.items.length > 0}
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-6 shadow-sm dark:shadow-none">
					<h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">Tasks ({contributions.tasks.completed}/{contributions.tasks.total} completed)</h4>
					<div class="space-y-2">
						{#each contributions.tasks.items as task}
							<div class="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/30">
								<div class="flex items-center gap-3 flex-1 min-w-0">
									<input
										type="checkbox"
										checked={task.completed}
										disabled
										class="rounded border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500"
									/>
									<span class="text-sm text-gray-900 dark:text-slate-200 truncate {task.completed ? 'line-through text-gray-400 dark:text-slate-500' : ''}">
										{task.title}
									</span>
								...
								{#if task.completedAt}
									<span class="text-xs text-gray-500 dark:text-slate-500 ml-2">
			...
			{#if contributions.habits.items.length > 0}
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-6 shadow-sm dark:shadow-none">
					<h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">Habits ({contributions.habits.logs} total logs)</h4>
					<div class="space-y-2">
						{#each contributions.habits.items as habit}
							<div class="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/30">
								<span class="text-sm text-gray-900 dark:text-slate-200">{habit.name}</span>
								<span class="text-xs text-gray-500 dark:text-slate-500">{habit.logCount} logs</span>
							</div>
						{/each}
					</div>
				</div>
								...
								<div class="flex-1">
									...
									<div class="text-xs text-gray-500 dark:text-slate-500 mt-1">
										...
										{#if session.completed}
											<span class="ml-2 text-emerald-700 dark:text-emerald-400">✓ Completed</span>
										{/if}
									</div>
								</div>
							</div>
			...
			{#if contributions.keyResults.length > 0}
				<div class="rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-6 shadow-sm dark:shadow-none">
					<h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">Key Results Progress</h4>
					<div class="space-y-4">
						{#each contributions.keyResults as kr}
							<div>
								<div class="flex items-center justify-between mb-2">
									<span class="text-sm font-medium text-gray-900 dark:text-slate-200">{kr.title}</span>
								...
								<div class="flex items-center justify-between mt-1 text-xs text-gray-500 dark:text-slate-500">
